...
t.test(DataframeFundos$AlaskaBlack, DataframeFundos$DynamoCougar, paired = T)

summary(DataframeFundos)

mean(DataframeFundos$AlaskaBlack)
mean(DataframeFundos$DynamoCougar)
summary(DataframeFundos)
...
tabelacruzada <- table(Antes = DataframeMcnemar$AntesPandemia, Depois = DataframeMcnemar$AposPandemia)
...
mean(DataframeWilcoxon$AntesPandemia)
mean(DataframeWilcoxon$AposPandemia)
...
shapiro.test(DataframeMannWhitney$ROE) ### Para fins didáticos, vamos considerar que ela não apresentou distribuição normal, sendo o Mann-Whitney adequada

wilcox.test(ROE ~ ADR, data = DataframeMannWhitney)

summary(DataframeMannWhitney)
